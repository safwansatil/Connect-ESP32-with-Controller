...
// ----- RPM Limit Parameters -----
const int maxRPM = 500;   // Maximum RPM of the motor (adjust this value as needed)
const int targetRPM = 400; // Desired RPM limit

// Map RPM to PWM
int mapRPMtoPWM(int rpm) {
  return (rpm / (float)maxRPM) * maxSpeed;
}
...
void updateMotorOutputs() {
  ...
  // Apply the RPM limit
  int limitedSpeed = mapRPMtoPWM(targetRPM);
  ...
  if (currentSpeed != 0) {
    ...
    if (currentSpeed > 0) { // Forward motion
      ...
      if (steerNorm > 0) {
        leftMotor  = limitedSpeed - turnAdjustment;
        rightMotor = limitedSpeed + turnAdjustment;
      } else {
        leftMotor  = limitedSpeed + turnAdjustment;
        rightMotor = limitedSpeed - turnAdjustment;
      }
      leftMotor  = constrain(leftMotor, 0, limitedSpeed);
      rightMotor = constrain(rightMotor, 0, limitedSpeed);
...
void processGamepad(ControllerPtr ctl) {
  ...
  if (circlePressed && r1Pressed) {
    ...
    if (r2Pressed) { // Forward acceleration.
    ...
    else if (l2Pressed) { // Reverse acceleration.
